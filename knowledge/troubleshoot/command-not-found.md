...
官方 FAQ：Hermes 預設 source ~/.bashrc，其他 shell 要在 config.yaml 的 terminal.shell\_init\_files 加入你的初始化檔。

### nvm、pyenv 裝的 node / python 也找不到？

同樣在 config.yaml 的 terminal.shell\_init\_files 加入 ~/.nvm/nvm.sh 等初始化檔。
